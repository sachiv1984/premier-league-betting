...
function detectPremierLeagueTeams(records) {
  // Count how many matches each team plays
  ...
  // Premier League teams play 38 matches per season (19 home + 19 away)
  // We'll be flexible and look for teams with 35-40 matches to account for incomplete seasons
  const premierLeagueTeams = Object.keys(teamMatchCounts)
    .filter(team => {
      const matchCount = teamMatchCounts[team];
      return matchCount >= 30 && matchCount <= 40; // Flexible range for incomplete seasons
    })
    .sort();
